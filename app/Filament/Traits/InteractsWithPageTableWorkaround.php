...
trait InteractsWithPageTableWorkaround
{
    ...
    protected function getTablePageInstance(): HasTable
    {
        ...
        foreach ([
            ...
            $page->{$property} = $this->normalizeTablePagePropertyValue($property, $value);
        }
    ...
    protected function normalizeTablePagePropertyValue(string $property, mixed $value): mixed
    {
        return match ($property) {
            // Filament page tables type this property as `array`, so Livewire `null`
            // payloads must be normalized before assignment.
            'tableColumnSearches', 'paginators' => is_array($value) ? $value : [],
            'tableSearch' => $value ?? '',
            default => $value,
        };
    }
